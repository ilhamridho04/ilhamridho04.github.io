...
module.exports = function (app) {
    ...
    app.get('/', isUserAllowed, function (req, res) {
        ...
        conn.connect().then(() => {
            ...
            var username = sess.user['users.username'];
            // Let's add an IP address to ether2
            conn.write('/ip/hotspot/user/print', '=count-only')
                .then((data) => {
                    //res.json(data);
                    var count = data[0];
                    res.render('Dashboard/index', { count, username });
    ...
    app.get('/gplay', function (req, res) {
        res.locals = { title: 'Aplikasi Details' };
        gplay.app({ appId: 'com.forevernine.ikpure.android' }).then((data) => {
            if (err) throw err;
            let text = data.description;
            let Summarizer = new SummarizerManager(text, 3);
            let short_description = Summarizer.getSummaryByFrequency().summary;
            res.render('Gplay/tambah', { data, short_description });
        }).catch((e) => {
            res.render('Gplay/tambah', { data, short_description });
        });
    });
    app.get('/gplay-add', function (req, res) {
        res.locals = { title: 'Aplikasi Details' };
        res.render('Gplay/tambah');
    });
    app.post('/app-detail', function (req, res) {
        res.locals = { title: 'Aplikasi Details' };
        gplay.app(req.body).then((data) => {
            let text = data.description;
            let Summarizer = new SummarizerManager(text, 3);
            let Summarizer_short_desc = Summarizer.getSummaryByFrequency().summary;
            let short_description = JSON.stringify(Summarizer_short_desc);
            // console.log(data);
            res.send({ data, short_description });
        }).catch((e) => {
    ...
    app.get('/apkpure', function (req, res) {
        ...
        gplay.list({
            ...
            //res.json(data);
            res.render('Gplay', { data });
        });
    });
    app.get('/hotspot-user', isUserAllowed, function (req, res) {
        res.locals = { title: 'Hotspot User' };
        conn.connect().then(() => {
            // Connection successful
